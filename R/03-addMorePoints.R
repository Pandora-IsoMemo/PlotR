                ...
                fluidRow(
                  ...
                  column(3,
                         ...
                         colourInput(ns("pointColor"), label = "Point color",
                                     value = '#002350'),
                         sliderInput(ns("pointSize"), label = "Point size",
                                     value = 1, min = 0.1, max = 5, step = 0.1),
                         selectInput(ns("pointSymbol"), label = "Point symbol",
                                     choices = pchChoices(), selected = 19),
                         colourInput(ns("textColor"), label = "Text color",
                                     value = '#002350'),
                         sliderInput(ns("textSize"), label = "Text size",
                                     value = 1, min = 0.1, max = 5, step = 0.1),
                         selectInput(ns("fontType"), label = "Font type",
...
addMorePoints <- function(input, output, session, savedData) {
  ...
  observe({
    req(morePoints())
    updateTextInput(session, "pointName", value = paste("Point", (length(morePoints()) + 1)))
    updateSelectInput(session, "pointsToRemove", choices = names(morePoints()), selected = NA)
    updateSelectInput(session, "activePoint", choices = names(morePoints()),
                      selected = names(morePoints())[length(morePoints())])
  })
  ...
  observeEvent(input$activePoint, {
    ...
    for (element in c("pointColor", "pointSize", "pointSymbol", "hidePointLabel",
                      "textColor", "textSize", "fontType")){
  ...
  observe({
    req(input$activePoint)
    # more points
    activePlotStyle$morePoints[[input$activePoint]]$pointColor <- input$pointColor
    activePlotStyle$morePoints[[input$activePoint]]$pointSize <- input$pointSize
    activePlotStyle$morePoints[[input$activePoint]]$pointSymbol <- input$pointSymbol
    activePlotStyle$morePoints[[input$activePoint]]$hidePointLabel <- input$hidePointLabel
    activePlotStyle$morePoints[[input$activePoint]]$textColor <- input$textColor
    activePlotStyle$morePoints[[input$activePoint]]$textSize <- input$textSize
    activePlotStyle$morePoints[[input$activePoint]]$fontType <- as.numeric(input$fontType)
  })
...
missingPointValues <- function(point) {
  missingValuesAt <- function(label) {
    (is.na(point[[paste0(label, "min")]]) || is.na(point[[paste0(label, "max")]])) && is.na(point[[label]])
  }
