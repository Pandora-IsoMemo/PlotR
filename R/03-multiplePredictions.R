                ...
                tabsetPanel(
                  ...
                  tabPanel(
                    ...
                    fluidRow(
                      ...
                      column(6,
                             tags$br(),
                             sliderInput(ns("xRangeCorr"), label = "Underlying X Range",
...
multiplePredictions <- function(input, output, session, savedData, loadedFiles) {
  ...
  observeEvent(input$activePlots, {
    ...
    updateSliderInput(session, "xRangeCorr",
                      min = getCommonX(activePlotsData(), what = "min"),
                      max = getCommonX(activePlotsData(), what = "max"),
                      value = c(getCommonX(activePlotsData(), what = "min"),
  ...
  observeEvent(input$predictY, {
    ...
    withProgress(
      ...
      value = 0, {
        predictedEstimationList(
          lapply(activePlotsData(), function(x) {
            incProgress(1 / length(activePlotsData()) * 0.9)
            predictAdditionalData(moreMean = moreX()$mean,
                                  moreSD = moreX()$sd,
    ...
    withProgress(
      ...
      value = 0, {
        predictedEstimationList(
          lapply(activePlotsData(), function(x) {
            incProgress(1 / length(activePlotsData()) * 0.9)
            predictAdditionalData(moreMean = moreXUploaded()$X,
  ...
  observe({
    ...
    predictedEstimation(
      lapply(predictedEstimationList(), function(x) {
        if (input$aggPrediction) colMeans(x) else x
      }) %>%
        bind_rows(.id = "name")
    )
  })
  ...
  observeEvent(input$deriveX, {
    ...
    derivedExplanatoryList(lapply(activePlotsData(), function(x) {
      deriveExplanatory(moreMean = moreY()$mean,
                        moreSD = moreY()$sd,
  ...
  observeEvent(input$deriveXUploaded, {
    ...
    derivedExplanatoryList(lapply(activePlotsData(), function(x) {
      deriveExplanatory(moreMean = moreYUploaded()$Y,
  ...
  observe({
    ...
    derivedExplanatory(
      lapply(derivedExplanatoryList(), function(x) {
        if (input$aggExplanatory) colMeans(x) else x
      }) %>%
        bind_rows(.id = "name")
    )
  })
  ...
  callModule(dataExport, "exportPredictedY", dat = dataPredictedY, filename = "predictedYData",
             useXOptions = FALSE)
  ...
  callModule(dataExport, "exportDerivedX", dat = dataDerivedX, filename = "derivedXData",
             useXOptions = FALSE)
}
