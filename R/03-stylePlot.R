                ...
                fluidRow(
                  column(3,
                         ...
                         selectInput(inputId = ns("sideXAxis"),
                                     ...
                                     selected = 1),
                         selectInput(inputId = ns("sideYAxis"),
                  ...
                  column(3,
                         h4("Titles"),
                         selectInput(inputId = ns("labelName"),
                                     label = "Select label",
                                     choices = c("plot title" = "plotTitle",
                                                 "x axis" = "xAxisLabel",
                                                 "y axis" = "yAxisLabel"),
                                     selected = NA),
                         textInput(ns("text"), label = "Text", value = NULL),
                         selectInput(ns("fontType"), label = "Font type",
                                     choices = fontChoices(), selected = NULL),
                  ...
                  column(3,
                         h4("Data Points"),
                         selectInput(inputId = ns("pointName"),
                                     label = "Select points",
                                     choices = c("data points" = "dataPoints",
                                                 "data outliers" = "dataOutliers",
                                                 "model outliers" = "modelOutliers"),
                                     selected = NA),
                         checkboxInput(inputId = ns("pointHide"),
                                       label = "Hide points",
                                       value = FALSE, width = "100%"),
                         selectInput(inputId = ns("pointSymbol"),
                                     label = "Symbol",
                                     choices = pchChoices(),
                                     selected = 19),
                         colourInput(inputId = ns("pointColor"),
                                     label = "Color",
                                     value = '#002350'),
                         ...
                         conditionalPanel(
                           ...
                           ns = ns,
                           colourInput(inputId = ns("pointColorBg"),
                                       label = "Background color",
                                       value = '#002350'),
                           sliderInput(inputId = ns("pointLineWidth"),
                                       label = "Thickness",
                  ...
                  column(3,
                         h4("Lines"),
                         selectInput(inputId = ns("lineName"),
                                     label = "Select lines",
                                     choices = c("prediction" = "predictionLine",
                                     ...
                                     selected = NA),
                         checkboxInput(inputId = ns("lineHide"),
                                       label = "Hide line",
                                       value = FALSE, width = "100%"),
                         selectInput(inputId = ns("lineType"),
                                     label = "Linetype",
                                     choices = c("solid" = "1", "dashed" = "2", "dotted" = "3",
                                                 "dotdash" = "4", "longdash" = "5", "twodash" = "6"),
                                     selected = 1),
                         colourInput(inputId = ns("lineColor"),
                                     label = "Color",
                                     value = '#1D60BD'),
                         sliderInput(inputId = ns("lineWidth"),
                                     label = "Thickness",
                                     min = 0, max = 20, value = 2)
                  )
                )
...
stylePlot <- function(input, output, session, savedData) {
  ...
  observeEvent(input$activePlot, {
    ...
    # titles
    updateSelectInput(session, "labelName", selected = getLastSelected(lastSelected$label, "plotTitle"))
    ...
    # data points
    updateSelectInput(session, "pointName", selected = getLastSelected(lastSelected$point, "dataPoints"))
    ...
    # lines
    updateSelectInput(session, "lineName", selected = getLastSelected(lastSelected$line, "predictionLine"))
